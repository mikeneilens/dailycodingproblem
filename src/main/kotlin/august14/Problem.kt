...
fun List<Int>.captureWater(wallHeight:Int = 0, trappedWater:Int = 0, total:Int = 0, remainingWaterTraps:List<Int> = listOf()):Int =
    when {
        isEmpty() && remainingWaterTraps.size <= 1 -> total
        isEmpty() -> total +  remainingWaterTraps.captureWater()
        first() < wallHeight -> drop(1).captureWater(wallHeight, trappedWater + (wallHeight - first()), total, listOf(first()) + remainingWaterTraps)
        else -> drop(1).captureWater(first(), 0, total + trappedWater, listOf(first()))
    }
