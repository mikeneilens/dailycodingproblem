...
fun List<IntRange>.minimumSetOfNumbers() =
    boundaries().let{(left, right) ->
        if (right > left) setOf(left, right) else setOf(right)
    }

fun List<IntRange>.boundaries() = Pair(minOf { it.last },maxOf { it.first })