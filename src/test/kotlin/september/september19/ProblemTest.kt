...
class ProblemTest: StringSpec({
    ...
    "unbiased random gives heads if random number sequence is always 1,0" {
        var n = 0
        val biasedRandom = listOf(1,0).fixedRandomGenerator()
    ...
    "unbiased random gives tails if random number sequence is always 0,1" {
        var n = 0
        val biasedRandom = listOf(0,1).fixedRandomGenerator()
...
fun List<Int>.fixedRandomGenerator():()->Int {
    var n = 0
    return { get(n++).also { n = n % size } }
}